...
const copyDirectory = join(process.cwd(), '_copy')
...
export function getAbout(fields, locale) {
  const lang = locale.split('-')[0]
  const filename = `index.${locale}.md`
  const fullPath = join(copyDirectory, 'about', filename)
  const fallBackPath =
    lang === 'zh' ? join(copyDirectory, 'about', 'index.zh-TW.md') : join(copyDirectory, 'about', 'index.md')

  const fileContents = fs.existsSync(fullPath, 'utf8')
