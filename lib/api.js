...
export function getPostBySlug(slug, fields = [], locale) {
  ...
  const filename = locale === defaultLocale || !locale ? 'index.md' : `index.${locale}.md`
...
export function getAllPostPaths(locales) {
  ...
  for (let slug of slugs) {
    for (let locale of locales) {
      let fullpath = join(postsDirectory, slug, locale === defaultLocale ? 'index.md' : `index.${locale}.md`)
      if (!fs.existsSync(fullpath)) {
        continue
      }
